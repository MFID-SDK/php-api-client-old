...
class Curl
{
    ...
    public function post($url, $data = array(), $makeup_data = true)
    {
        ...
        $this->setOpt(CURLOPT_POSTFIELDS, $makeup_data ? $this->postfields($data) : $data);
        $this->setOpt(CURLOPT_CONNECTTIMEOUT, 3);
        $this->setOpt(CURLOPT_TIMEOUT,        5);
        return $this->exec();
    }

    public function download ($url, $file, &$status = false)
    {
        $fh             = fopen($file, 'w+');
        $this->base_url = $url;
        $this->url      = $url;
        ...
        if (is_file($file) && filesize($file) > 0)
        {
            ...
            $filedata = explode("\r\n\r\n", $filedata, 2);
            $status   = 0;
            if (preg_match_all("/HTTP\/1\.[01] (\d\d\d)/", $filedata[0], $matches)) {$status = $matches[1][sizeof($matches[1]) - 1];}
            file_put_contents($file, $filedata[1]);
            return true;
        }
        return false;
    }

    public function put($url, $data = array(), $makeup_data = true)
    {
        $this->base_url = $url;
        $this->url = $url;
        $this->setOpt(CURLOPT_URL, $this->url);
        $this->setOpt(CURLOPT_CUSTOMREQUEST, 'PUT');
        $put_data = $makeup_data ? http_build_query($data) : $data;
